...
export default {
  ...
  async created() {
    ...
    ipcRenderer.on("update-downloaded", (e, info) => {
      this.$toast.success(`New update downloaded! ${info.releaseName} 💖`, {
        timeout: 7500,
      });
      this.$toast(`Update will be installed once you exit Endeavor.`, {
        timeout: 7500,
      });
  ...
  methods: {
    async login(username, password) {
      ...
      try {
        ...
        loginResult = await this.sharedStore.eLearn.login(
          username,
          password,
          (log) => {
            this.sharedStore.fullPageLoadLog.push({
              text: log,
              time: Date.now(),
            });
          }
        ...
        console.warn(
          "An error occured trying to retrieve login credentials: " + err
        );
        return false;
      }
