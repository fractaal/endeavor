...
import {
  transformHtml,
  transformUrl,
  transformUrlWithoutChangingBaseURL,
...
// Types
import { PagesData } from "./objects/page-data";
...
function convertRawTimeValuesToDate(object: any): any {
  for (const key in object) {
    ...
    if (
      (key.match("date") || key.match("time")) &&
      !key.match("timeduration")
    ) {
...
export class ELearn implements eLearnInterface {
  ...
  constructor() {
    this.notificationsStore = new Store({
      name: "notifications",
      defaults: {
    ...
    for (let i = 0; i <= maxRetries; i++) {
      try {
        result = await promiseTimeout(
          7500,
          this._login(username, password, update)
        );
      } catch (err) {
        if (err !== "TIMEOUT") {
          ...
          console.warn(
            `[elearn-api] Login attempt ${i + 1} failed - ${err} - retrying`
          );
          Bus.$emit(
            "toast",
            `Login request timed out! ${i + 1}/${maxRetries} retries`,
            {},
            "warning"
          );
  ...
  private async _login(
    ...
    try {
      // To obtain a cookie
      await client(
        `${baseurl}/lib/ajax/service.php?info=tool_mobile_get_public_config`
      );
      ...
      try {
        ...
        Bus.$emit(
          "toast",
          `Login failed. Incorrect password! ${err}`,
          {},
          "error"
        );
        return false;
      ...
      const siteInfoRequest: Record<string, any> = await this.wsFunction(
  ...
  async wsFunctionRaw(form: Record<string, any>, token?: string): Promise<any> {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "[elearn-api] wsFunctionRaw response parse failure " + err
        );
        return null;
      }
    } catch (err) {
  ...
  async getCourses(): Promise<CourseMetadata[]> {
    ...
    const res = await this.wsFunction("core_enrol_get_users_courses", {
  ...
  async getCourseInfo(courseid: number, overwrite = false): Promise<Section[]> {
    ...
    const [entries, forums, quizzes, assignments] = await Promise.all([
      a,
      b,
      c,
      d,
    ...
    for (const section of entries as Section[]) {
      ...
      for (const module of section.modules) {
        ...
        if (module.description) {
          module.description = transformHtml(
            module.description,
            (await this.getSession()).token
          );
        }
        ...
        if (info[module.modname]) {
          ...
          const data: Quiz & Assignment & Forum = findInArray(
            info[module.modname],
            module.instance
          );
          ...
          if (data) {
            ...
            if (data.introattachments) {
              for (const content of data.introattachments)
                content.fileurl = transformUrlWithoutChangingBaseURL(
                  content.fileurl,
                  session.token
                );
            }
              ...
              if (key.match("date") || key.match("time")) {
                ...
                if (data[key] == 0) {
                  ...
                  if (key.match("timelimit")) {
                    module[
                      "timelimitformatted"
                    ] = `⌛ Good for ${formatDistanceStrict(
                      Date.now(),
                      Date.now() - data[key]
                    )}`;
                  } else if (key.match("timeopen")) {
                    module.timeopenformatted = `✔ Opens ${format(
                      data[key],
                      "hh:mma, MMMM dd"
                    )}`;
                  } else if (key.match("timeclose")) {
                    module.timecloseformatted = `❌ Closes ${format(
                      data[key],
                      "hh:mma, MMMM dd"
                    )}`;
                  }
            ...
            const style = (key) => {
              ...
              module.duedateformatted = format(
                module[key],
                "hh:mma, MMMM dd, yyyy"
              );
              /**
  ...
  async getSection(courseid: number, sectionNumber: number): Promise<Section> {
    const course: CourseMetadata = findInArray(
      this.cache.coursesMetadata,
      courseid
    );
  ...
  async getTimeline(): Promise<Event[]> {
    ...
    for (let event of events) {
      ...
      const correspondingModule = await this.getModule(
        event.course.id,
        event.instance
      );
  ...
  async toggleModuleCompletion(module: Module): Promise<boolean> {
    ...
    const response: any = await this.wsFunction(
      "core_completion_update_activity_completion_status_manually",
      { cmid: module.id, completed: targetValue ? 1 : 0 }
    );
    ...
    try {
      if (response.status == true) {
        ...
        Bus.$emit(
          "toast",
          `Couldn't toggle completion for ${module.name} - ${response.message}`,
          {},
          "error"
        );
        return false;
      ...
      console.warn(
        `[elearn-api toggleCompletion] FAIL for module ${module.name} - ${err}`
      );
      Bus.$emit(
        "toast",
        `Couldn't toggle completion for ${module.name} - ${err}`
      );
      return false;
    }
  }
  ...
  async initialize(update: Function): Promise<void> {
    if (this.cacheExists()) {
      console.log(
        `[elearn-api] Skipping initial cache build step - cache already exists!`
      );
      update("💖 Complete!");
      return;
    } else {
  ...
  async buildCache(
    update: Function = (msg) => console.log(msg)
  ): Promise<ELearn["cache"]> {
    ...
    const courses: any = await Promise.all(
      coursesMetadata.map((course) => {
        return new Promise((resolve) => {
          if (
            getCourseVisibility(course.id) ||
            sharedStore.settings.loadHiddenCourseData
          ) {
  ...
  async getFeedback(
    ...
    if (type == "quiz") {
      grade = parseInt(
        (await this.wsFunction("mod_quiz_get_user_best_grade", { quizid: id }))
          .grade
      );
      if (isNaN(grade)) grade = 0;
    } else if (type == "assign") {
      const response = await this.wsFunction(
  ...
  async getLessonPages(
    lessonid: string,
    updateFunction = (msg: string) => console.log(msg)
  ): Promise<PagesData> {
    ...
    try {
      ...
      const pageMetadata: Page[] = (
        await this.wsFunction("mod_lesson_get_pages", { lessonid })
      ).pages;
      const usablePageMetadata = pageMetadata.filter(
        (page) => page.page.typestring == "Content"
      );

      // Get page data and then format this data onto the PageData array
      await Promise.all(
        usablePageMetadata.map(async (pageMetadata, idx) => {
          updateFunction(
            `Retrieving page data (${idx + 1} of ${usablePageMetadata.length})`
          );
          const data = await this.wsFunction("mod_lesson_get_page_data", {
            lessonid,
            pageid: pageMetadata.page.id,
          });
          finalPageData[idx] = {
      ...
      console.warn(
        `[elearn-api] Failed to get lesson pages for ID ${lessonid} reason: ${err}`
      );
      return;
    }
  ...
  async getBookPages(
    ...
    const parseStructureNode = async (
      raw,
      item: BookPage
    ): Promise<BookPage> => {
      ...
      for (const element of raw.contents) {
        try {
          ...
          if (
            item.href
              .match(/[0-9]+/g)[0]
              .match(path.basename(element.filepath))[0] ==
            item.href.match(/[0-9]+/g)[0]
          ) {
      ...
      modified.url = transformUrlWithoutChangingBaseURL(
        data.fileurl,
        session.token
      );
      let res;
      ...
      [res, modified.subitems] = await Promise.all([
        client(modified.url),
        Promise.all(
          modified.subitems.map((item) => parseStructureNode(raw, item))
        ),
      ]);
    ...
    if (course) {
      ...
      try {
        ...
        if (found) {
          ...
          console.warn(
            `[elearn-api] Failed to get book pages for ID ${bookid} - Book not found in course`
          );
          return null;
        }
      } catch (err) {
        console.warn(
          `[elearn-api] Failed to get book pages for ID ${bookid} - reason: ${err}`
        );
        return null;
      }
    } else {
      console.warn(
        `[elearn-api] Failed to get book pages for ID ${bookid} - course not found`
      );
      return null;
    }
  }
  ...
  cacheExists() {
    return (
      this.cache.buildTime !== undefined &&
