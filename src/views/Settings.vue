      ...
      <div class="settings-section">
        <div>
          ...
          <div class="theme-select">
            <ul>
              <li v-for="theme in sharedStore.themes" :key="theme.displayName" @click="theme.setActive()" :class="theme.displayName == sharedStore.settings.theme ? 'active' : ''">
                <div>
                  <div style="display: flex;">
                    <h2>{{theme.displayName}}</h2><h2 style="font-weight: 200; opacity: 0.5; padding-left: 7px;">— {{theme.author}}</h2>
                  </div>
                  <span>
                    <p style="font-weight: 200; opacity: 0.5;">VERSION {{theme.version.toUpperCase()}}</p>
        ...
        <div>
          <div>
            ...
            <p class="nospacing">
              Change the current font face. This feature may require an internet connection <i>(If you're using Google Fonts.)</i>
            </p>
          </div>
          <br>
          <div style="display: flex; align-items: center;">
            ...
            <button class="roundButton" @click="resetFont"><fai icon="redo"/></button>
          </div>
        ...
        <div>
          ...
          <br>
          <div style="display: flex; align-items: center;">
            <button @click="decreaseZoom" class="roundButton"><fai icon="minus"/></button>
            <h2 style="margin: 0px 15px 0px 15px;">{{(sharedStore.settings.zoomLevel*100).toFixed(0)}}%</h2>
            <button style="margin-right: 5px;" @click="increaseZoom" class="roundButton"><fai icon="plus"/></button>
            <button style="margin-right: 20px;" class="roundButton" @click="resetZoom"><fai icon="redo"/></button>
          </div>
        </div>
        <div style="display: flex; align-items: center;">
          <div class="toggle">
            <input type="checkbox" class="check" v-model="sharedStore.settings.showActivityDescriptionsOnCards">
            <b class="b switch"></b>
            <b class="b track"></b>
          </div>
          <div style="margin-left: 25px;">
            <h2 class="nospacing" style="font-weight: 400;">SHOW ACTIVITY DESCRIPTIONS</h2>
            <p class="nospacing">
              Show activity descriptions in their preview cards.
              <br><br>
              If you have many long activities, turning this off may reduce the time spent scrolling.
            </p>
          </div>
        </div>
        <div>
          <div>
            ...
            <p class="nospacing">
              ...
              <br><br>
              If you have many long activities, switching to 2-column view may help. 
            </p>
          </div>
          <br>
          <div style="display: flex; align-items: center;">
            <endeavor-button :styling="sharedStore.settings.numColumnsInCourseView == 1 ? 'active' : ''" @click="setToOneColumn">1 column</endeavor-button>
            <endeavor-button :styling="sharedStore.settings.numColumnsInCourseView == 2 ? 'active' : ''" @click="setToTwoColumns">2 columns</endeavor-button>
          </div>
        </div>
      </div>
